 ...
 *
 * `prefer` breaks an EXACT TIE, and a tie is not an edge case — it happens
 * whenever the two copies are the same distance apart, which for a TWO-image
 * gallery is every single move. With count 2 the fold always landed on -1, so
 * `next` animated backwards, `previous` did the identical thing, and the second
 * image was only ever drawn to the left of the first: nothing existed to the
 * right to move into, and the carousel read as stuck. A product with exactly
 * two photos is completely ordinary, so this was not a corner.
 *
 * At a tie both copies are equally near by definition, so choosing the one in
 * the direction of travel is free — it just has to be chosen deliberately
 * rather than falling out of the sign of a modulo.
 */
export function nearestDelta(index, position, count, loop, prefer = 1) {
  const raw = index - position;
  if (!loop || count < 2) return raw;
  const half = count / 2;
  const folded = (((raw + half) % count) + count) % count;
  const delta = folded - half;
  // delta === -half means the two copies straddle the halfway point exactly.
  return delta === -half && prefer > 0 ? half : delta;
}
...
export function useCarousel(count, { loop = true, gap = 0 } = {}) {
  ...
  const goTo = useCallback(
    (target, prefer = 1) => {
      ...
      // step forwards instead of winding all the way back. `prefer` decides an
      // exact tie, so on a two-image gallery "next" goes forwards and
      // "previous" goes backwards instead of both doing the same thing.
      animateTo(
        loop
          ? position.current +
              nearestDelta(settled, position.current, count, true, prefer)
          : settled
      );
    },
  ...
  const endDrag = useCallback(
    (event) => {
      ...
      //
      // `heading` is also the tie-break: on a two-image gallery both copies are
      // equidistant, so without it a backward swipe would settle forwards and
      // the gesture would fight the finger.
      goTo(Math.round(state.from) + heading * steps, heading || 1);
    },
    [goTo]
  );
